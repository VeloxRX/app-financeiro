...
async function seed() {
    ...
    for (const cat of categories) {
        const existing = await database_1.pool.query('SELECT id FROM categories WHERE name = $1 AND is_system = TRUE AND user_id IS NULL', [cat.name]);
        if (existing.rows.length === 0) {
            await database_1.pool.query('INSERT INTO categories (id, user_id, name, icon, color, is_system) VALUES ($1, NULL, $2, $3, $4, TRUE)', [(0, uuid_1.v4)(), cat.name, cat.icon, cat.color]);
