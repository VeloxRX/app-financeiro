...
export async function autoCategorizeTransaction(
  description: string, amount: number, userId: string
): Promise<{ category_id: string | null; confidence_score: number }> {
  const descLower = description.toLowerCase();

  // First: check user's historical categorization
  const historical = await query<{ category_id: string; cnt: string }>(
  ...
  if (bestMatch) {
    const cat = await query<{ id: string }>(
      "SELECT id FROM categories WHERE name = $1 AND (user_id = $2 OR is_system = TRUE) LIMIT 1",
      [bestMatch, userId]
    );
    if (cat[0]) {
  ...
  // Fallback: "Outros"
  const outros = await query<{ id: string }>(
 ...
 */
export async function detectAnomalousSpending(
  transaction: { amount: number; category_id: string; type: string },
  userId: string
): Promise<{ is_anomalous: boolean; avg_historical: number; deviation_percent: number }> {
  if (transaction.type !== 'expense') {
    return { is_anomalous: false, avg_historical: 0, deviation_percent: 0 };
  }

  const stats = await query<{ avg: string; stddev: string }>(
    `SELECT AVG(amount) as avg, COALESCE(STDDEV(amount), 0) as stddev
     FROM transactions
  ...
  if (avg === 0) return { is_anomalous: false, avg_historical: 0, deviation_percent: 0 };
 ...
 */
export async function generateMonthlyInsight(userId: string, month: number, year: number): Promise<string> {
  // Check cached insight first
  const cached = await query<{ insight: string }>(
    'SELECT insight FROM ai_insights WHERE user_id = $1 AND month = $2 AND year = $3',
    [userId, month, year]
  );
  if (cached[0]) return cached[0].insight;

  const monthStart = `${year}-${String(month).padStart(2, '0')}-01`;
  const monthEnd = new Date(year, month, 0).toISOString().split('T')[0];

  // Current month data
  const currentData = await query<{ type: string; total: string }>(
  ...
  const topCats = await query<{ name: string; total: string }>(
  ...
  const prevExpenses = await query<{ total: string }>(
  ...
  if (parts.length > 0) {
    await query(
      `INSERT INTO ai_insights (id, user_id, month, year, insight)
       VALUES (gen_random_uuid(), $1, $2, $3, $4)
       ON CONFLICT (user_id, month, year) DO UPDATE SET insight = $4, generated_at = NOW()`,
      [userId, month, year, insight]
    );
  }
...
export async function forecastBalance(
  userId: string, monthsAhead: number = 6
): Promise<Array<{ date: string; predicted_balance: number; confidence: number }>> {
  // Get last 6 months of monthly net
  const history = await query<{ month: string; net: string }>(
  ...
  const avgMonthly = nets.reduce((sum, val, idx) => {
  ...
  const forecast: Array<{ date: string; predicted_balance: number; confidence: number }> = [];
  const now = new Date();
  
  for (let i = 1; i <= monthsAhead; i++) {
    const d = new Date(now.getFullYear(), now.getMonth() + i, 1);
    const monthStr = `${String(d.getMonth() + 1).padStart(2, '0')}/${d.getFullYear()}`;
    currentBalance += avgMonthly;
    forecast.push({
      date: monthStr,
      predicted_balance: Math.round(currentBalance * 100) / 100,
      confidence: Math.max(0.5, 1 - (i / monthsAhead) * 0.5),
    });
  }
 ...
 */
export async function generateSavingsTips(userId: string): Promise<Array<{
  category: string;
  current_avg: number;
  potential_saving: number;
  tip: string;
}>> {
  const spending = await query<{ name: string; avg: string; total: string }>(
    `SELECT c.name, AVG(t.amount) as avg, SUM(t.amount) as total
  ...
  const tips: Array<{ category: string; current_avg: number; potential_saving: number; tip: string }> = [];
